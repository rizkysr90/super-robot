...
type TenantPermissionFinder struct {
	db *sql.DB
}

func NewTenantPermissionFinder(db *sql.DB) *TenantPermissionFinder {
	return &TenantPermissionFinder{
		db: db,
	}
}

func (t *TenantPermissionFinder) FindByTenantRoleID(ctx context.Context, tenantRoleIds []string) ([]store.TenantPermissionData, error) {
	query := `
