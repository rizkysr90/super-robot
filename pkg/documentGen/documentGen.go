...
// Document Generator defines the interface for document generation functionality
type DocumentGenaratorInterface interface {
	// Generate document label pricing
	LabelPricing(productData *store.ProductData) ([]byte, error)
}
...
func (dc *DocumentGenerator) LabelPricing(productData *store.ProductData) ([]byte, error) {
	// Generate and save barcode image
	tempFile, err := generateBarcode(productData.ProductID)
	if err != nil {
		return nil, err
	}
	// Create PDF with barcodes
	pdfBytes, err := createPDF(productData, tempFile)
	if err != nil {
		return nil, err
	}
	return pdfBytes, nil
...
func createPDF(product *store.ProductData, barcodeImagePath string) ([]byte, error) {
	...
	// Load font
	fontPath := filepath.Join("internal", "assets", "fonts", "Arial.ttf")
